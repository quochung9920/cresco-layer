(function () {
		...
		box.innerHTML = '' +
			...
			'<div class="cresco-ai-panel__body">' +
				'<section data-cresco-ai-pane="prepare">' +
					...
					'<label class="cresco-ai-reference"><input type="file" accept="image/*" data-cresco-ai-reference><span><strong>Reference image (optional)</strong><small>Choose it here so the JSON records the reference. Attach the same image to your AI chat.</small></span></label>' +
				...
				'<section data-cresco-ai-pane="import" hidden>' +
					...
					'<label class="cresco-ai-field"><span>AI result</span><textarea data-cresco-ai-import rows="10" placeholder="Paste cresco-layer-patch/v1 or cresco-layer-ai-result/v1. Markdown code fences and common AI wrappers are accepted."></textarea></label>' +
	...
	function referenceMetadata(file) {
		if (!file) return { provided: false, delivery: 'attach-separately' };
		return { provided: true, name: String(file.name || ''), type: String(file.type || ''), size: Number(file.size || 0), delivery: 'attach-separately' };
	}
	...
	function renderReady(box, pkg) {
		...
		if (summary) summary.innerHTML = '<strong>Context ready</strong><span>' + esc(pkg.target && pkg.target.type || 'Element') + ' · ' + esc(pkg.target && pkg.target.id || '') + '</span><span>' + (pkg.currentInterface && pkg.currentInterface.elementCount || 0) + ' source elements · ' + (pkg.layoutGraph && pkg.layoutGraph.nodes ? pkg.layoutGraph.nodes.length : 0) + ' layout nodes</span>';
	}
	...
	window.CrescoLayerAIPanel = {
		version: '1.0.0', open: openPanel, close: closePanel,
