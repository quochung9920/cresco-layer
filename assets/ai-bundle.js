(function () {
	...
	function targetNode(targetId) {
		...
		try {
			return doc.querySelector('[data-id="' + CSS.escape(String(targetId)) + '"],.elementor-element-' + CSS.escape(String(targetId)));
		} catch (e) { return null; }
	}
	function styleString(win, node) {
		...
		for (var i = 0; i < css.length; i++) {
			var key = css[i];
			if (key.indexOf('-webkit-') === 0) continue;
	...
	function cloneWithStyles(source, win) {
		...
		sourceNodes.forEach(function (node, i) {
			if (!cloneNodes[i] || node.nodeType !== 1) return;
			try { cloneNodes[i].setAttribute('style', styleString(win, node)); } catch (e) {}
		});
		return clone;
	}
	function rasterizeTarget(targetId) {
		return new Promise(function (resolve) {
			var node = targetNode(targetId);
			...
			var clone = cloneWithStyles(node, win);
			...
			img.onload = function () {
				try {
					...
					canvas.toBlob(function (png) {
						...
						png.arrayBuffer().then(function (buffer) { resolve({ name: 'current-desktop.png', data: new Uint8Array(buffer), width: width, height: height, source: 'elementor-preview-iframe-foreignobject' }); }).catch(function () { resolve(null); });
					}, 'image/png');
	...
	function taskMarkdown(pkg, raster) {
		var target = pkg.target || {}, task = pkg.task || {}, placement = pkg.placementContext || {}, output = pkg.outputContract || {};
		return [
			'# Cresco AI Task', '',
			'Goal:', task.request || '(No explicit request supplied)', '',
			'Target:', 'Post ' + (target.postId || 0), 'Element ' + (target.id || ''), 'Scope ' + (target.scope || ''), '',
			'Widget policy:', 'Use widget-guide.json and runtime-proven widget types only.', '',
			...
			'Output:', 'Preferred schema: ' + (output.preferredSchema || 'cresco-ai-mutation/v2'), 'Return only the intended mutation; do not echo source context.', '',
	...
	function build(pkg, referenceFile) {
		...
		return Promise.all([rasterizeTarget(targetId), referenceBytes(referenceFile)]).then(function (parts) {
			...
			var guide = { schema: 'cresco-widget-guide/v1', widgetIntelligence: pkg.widgetIntelligence || {}, constructionPlan: pkg.constructionPlan || {}, semanticBindings: pkg.semanticBindings || {}, structureGrammar: pkg.structureGrammar || {}, controlExamples: pkg.controlExamples || {} };
			var contract = pkg.outputContract || {};
			var manifest = {
				schema: 'cresco-ai-bundle/v1', pluginVersion: (window.crescoLayerEditor || {}).version || '', createdAt: new Date().toISOString(),
				target: pkg.target || {}, contextQuality: pkg.contextQuality || {}, raster: raster ? { file: raster.name, width: raster.width, height: raster.height, source: raster.source } : { file: null, status: 'unavailable' },
				reference: reference ? { file: reference.name } : { file: null }, files: []
			};
			var files = [
	...
	function exportBundle(pkg, referenceFile) {
		return build(pkg, referenceFile).then(function (result) {
			var target = pkg.target && pkg.target.id || 'target';
			downloadBlob('cresco-ai-bundle-' + target + '.zip', result.blob);
	...
	window.CrescoLayerAIBundle = {
		version: '1.0.0', build: build, export: exportBundle,
