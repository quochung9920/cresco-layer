...
function responsive_controls( bool $with_widescreen = true ): array {
	$slider = [ 'type' => 'slider', 'is_responsive' => true, 'size_units' => [ 'px', 'custom' ] ];
...
$spec = ( new ProfessionalCommerceProfile() )->spec();
...
foreach ( $widths as $control => $expected ) {
	...
	rf_assert( 'px' === $settings[ $control ]['unit'], "{$control} must use native px." );
	rf_assert( $expected === (float) $settings[ $control ]['size'], "{$control} must have the expected width." );
}
