...
foreach ($contract['containerPadding'] as $device => $padding) { p_assert(isset($padding['fluid'],$padding['fallbackPx']), "$device global padding fluid"); p_assert($padding === $contract['pageGutter'][$device], "$device global padding mirrors gutter"); p_assert(!isset($padding['fixedPx']), "$device global padding not zeroed"); }
