...
v_assert(in_array('content_width_above_control_range', array_column($bad['errors'], 'code'), true), 'explicit range violation reported');
...
$no_custom = $controls; $no_custom['container_padding']['size_units'] = ['px'];
$no_custom_result = (new ResponsiveFoundationValidator(new RuntimeControlResolver($no_custom)))->validate($spec);
v_assert(true === $no_custom_result['compatible'], 'older runtime remains compatible through explicit px fallback');
v_assert(in_array('container_padding_custom_unit_unsupported', array_column($no_custom_result['warnings'],'code'), true), 'custom unit fallback warning reported');
