...
final class AIMutationCompiler {
	...
	private function content_settings( string $intent, array $content ): array {
		...
		if ( preg_match( '/heading|headline|title/', $name ) ) {
			...
			if ( preg_match( '/^h[1-6]$/', $level ) ) { $out['header_size'] = $level; }
		} elseif ( preg_match( '/text-editor|paragraph|rich-text|^text$/', $name ) ) {
			if ( isset( $content['html'] ) ) { $out['editor'] = (string) $content['html']; }
			elseif ( '' !== $text ) { $out['editor'] = '<p>' . esc_html( $text ) . '</p>'; }
		} elseif ( preg_match( '/button|cta/', $name ) ) {
			if ( '' !== $text ) { $out['text'] = $text; }
			if ( isset( $content['url'] ) ) {
				$out['link'] = is_array( $content['url'] ) ? $content['url'] : [ 'url' => esc_url_raw( (string) $content['url'] ) ];
			}
		} elseif ( preg_match( '/image|media/', $name ) && is_array( $content['image'] ?? null ) ) {
			$out['image'] = $content['image'];
		} elseif ( preg_match( '/icon/', $name ) && is_array( $content['icon'] ?? null ) ) {
			$out['selected_icon'] = $content['icon'];
		}
		return $out;
	}
