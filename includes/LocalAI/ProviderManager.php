...
final class ProviderManager {
	...
	public function providers(): array {
		return [
			...
			'openai-compatible' => [
				'label' => 'OpenAI-compatible local API',
				'defaultEndpoint' => 'http://127.0.0.1:1234/v1',
				'apiStyle' => 'openai-compatible',
				'modelsPath' => '/models',
				'healthPath' => '/models',
	...
	public function summary(): array {
		...
		return [
			...
			'connection' => [
				'mode' => (string) $config['connectionMode'],
				'browserRequired' => 'browser' === (string) $config['connectionMode'],
	...
	public function test(): array {
		...
		if ( 'browser' === (string) $config['connectionMode'] ) {
			return [
				...
				'message' => 'Browser / Local Bridge mode must be tested from the Cresco Layer admin page on this computer.',
				'descriptor' => $this->browser_descriptor( $config ),
			];
		}
		...
		return [
			...
			'message' => 'Local AI endpoint responded successfully.',
		];
	}
	...
	public function models(): array {
		...
		return [
			'browserRequired' => false,
			'provider' => (string) $config['provider'],
			'models' => $this->normalize_models( (string) $config['provider'], $data ),
		];
	}
	...
	public function diagnostics(): array {
		$config = $this->settings->get();
		$checks = [];
		$checks[] = [ 'id' => 'enabled', 'label' => 'Local AI enabled', 'status' => $config['enabled'] ? 'pass' : 'warning', 'detail' => $config['enabled'] ? 'Enabled' : 'Disabled' ];
		$checks[] = [ 'id' => 'endpoint', 'label' => 'Local endpoint policy', 'status' => 'pass', 'detail' => (string) $config['endpoint'] ];
		$checks[] = [ 'id' => 'privacy', 'label' => 'Sensitive context redaction', 'status' => ! empty( $config['redactSensitiveContext'] ) ? 'pass' : 'fail', 'detail' => ! empty( $config['redactSensitiveContext'] ) ? 'Forced on' : 'Disabled' ];
		if ( 'browser' === (string) $config['connectionMode'] ) {
		...
		try {
			...
			$checks[] = [ 'id' => 'connection', 'label' => 'Connection', 'status' => 'pass', 'detail' => (string) ( $connection['latencyMs'] ?? 0 ) . ' ms' ];
		} catch ( \Throwable $error ) {
			$checks[] = [ 'id' => 'connection', 'label' => 'Connection', 'status' => 'fail', 'detail' => $error->getMessage() ];
			return [ 'ok' => false, 'browserRequired' => false, 'checks' => $checks ];
		}
		...
		try {
			...
			$checks[] = [ 'id' => 'models', 'label' => 'Model discovery', 'status' => 'fail', 'detail' => $error->getMessage() ];
		}
	...
	public function browser_descriptor( ?array $config = null ): array {
		...
		return [
			...
			'endpoint' => (string) $config['endpoint'],
			'healthUrl' => $this->url( $config, (string) $provider['healthPath'] ),
			'modelsUrl' => $this->url( $config, (string) $provider['modelsPath'] ),
			'chatUrl' => $this->url( $config, (string) $provider['chatPath'] ),
			'hasApiToken' => '' !== (string) ( $config['apiToken'] ?? '' ),
		];
	}

	private function provider( string $provider ): array {
		$providers = $this->providers();
		if ( ! isset( $providers[ $provider ] ) ) { throw new \InvalidArgumentException( 'Unsupported local AI provider.' ); }
		return $providers[ $provider ];
	}

	private function url( array $config, string $path ): string {
		return rtrim( (string) $config['endpoint'], '/' ) . '/' . ltrim( $path, '/' );
	}
